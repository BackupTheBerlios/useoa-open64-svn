...
!-------------------------------------------------
!         call print_results5('CG', class, na, 0, 0)
!         call print_results4(niter, nnodes_compiled, nprocs, tmax)
!         call print_results2(mflops, '          floating point') 
!         call print_results3(verified, npbversion, compiletime)
!         call print_results7(cs1, cs2, cs3, cs4, cs5, cs6, cs7)
!-------------------------------------------------
...
c---------------------------------------------------------------------
      if( nprocs .ne. num_procs )then
          if( me .eq. root ) then
             junk = 5
CC           write( *,9000 ) nprocs, num_procs
CC 9000     format(      /,'Error: ',/,'num of procs allocated   (', 
CC     >                 i4, ' )',
CC     >                 /,'is not equal to',/,
CC     >                 'compiled number of procs (',
CC     >                 i4, ' )',/   )
          endif
          call mpi_finalize(ierr)
          stop
 ...
 100  continue
          if( i .ne. 1 .and. i/2*2 .ne. i )then
              if ( me .eq. root ) then  
                 junk = 6
CC                write( *,* ) 'Error: num_proc_cols is ',
CC    >                         num_proc_cols,
CC    >                        ' which is not a power of two'
              endif
              call mpi_finalize(ierr)
              stop
          endif
...
c---------------------------------------------------------------------
      ...
      size = 1.0D0
c      ratio = rcond ** (1.0D0 / dfloat(n))
      ratio = rcond ** (1.0D0 / float(n))
      nnza = 0
...
! print_results stub
!      subroutine print_results(s1,c,n,
!     >           i1,i2,ni,nc,np,tm,mf,s2,v,npb,ct
...
!     >                      cs1, cs2, cs3, cs4, cs5, cs6, cs7)
       ...
       subroutine print_results(a1,a2,a3,a4,a5,
     > a6,a7,a8,a9,a10,a11,a12,a13,a14,
     > cs15,cs16,cs17,cs18,cs19,cs20,cs21)
       character*2 a1
       character a2
       integer a3,a4,a5
       integer a6,a7,a8
...
!      double precision function randlc(d1,d2)
!      double precision d1,d2
!      randlc = d1*d2
!      end function randlc

      subroutine timer_clear(i1)
      integer i1
      return
      end subroutine timer_clear

      subroutine timer_start(i1)
      integer i1
      return
      end subroutine timer_start

      double precision function timer_stop(i1)
      integer i1
      timer_stop = 1.0D0
      end function timer_stop

      double precision function timer_read(i1)
      integer i1
      timer_read = 0.5D0
      end function timer_read


!------- Bring in the MPI stubs -----------
