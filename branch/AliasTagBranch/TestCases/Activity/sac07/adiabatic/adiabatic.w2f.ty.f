...
C
      ...
      GO TO 10
1     CONTINUE
      KLAG = IFIX(FLAG + 2.0999999046)
      KLAG = MIN(MAX(KLAG, 1), 3)
      t__24 = KLAG
...
C
     ...
     >  *(TA ** 3) + CSXX(I, 12) *(TA ** 4) + CSXX(I, 13) *(TA ** 5) +
     >  CSXX(I, 14) *(TA ** 6) + CSXX(I, 15) *(TA ** 7))
        TA = T + TZ
1       CONTINUE
      END DO
...
C
      ...
      DO I = 1, t__49, 1
        ...
        AKE(I) = (PVRES / P)
45      CONTINUE
        AKBARL = AKBARL +((LOG(AKE(I) + 1.0000000134E-10) *(WEN(JW, I)
     >  / WTEN(JW))) / LOG(1.0E+01))
      END DO
...
26          CONTINUE
          ...
          CALL pvap((K), T, PVRES)
          PVK = PVRES / P
          SUMD = SUMD +(((PVK +(-1.0)) *(WEN(JW, K) / WTEN(JW))) /(PVK
     >  * VF + 1.0 - VF))
24        CONTINUE
        END DO
...
29      CONTINUE
        DT = -(FUNXON * SIGN(MAX(ABS(DFUNC), 9.9999997765E-03), DFUNC))
        DT = MAX(DL, MIN(DH, DT))
...
41          CONTINUE
          ...
          CALL pvap((K), T, PVRES)
          PVK = PVRES / P
          SUMD = SUMD +(((WEN(JW, K) / WTEN(JW)) *(PVK +(-1.0))) /(PVK
     >  * VF + 1.0 - VF))
39        CONTINUE
        END DO
        FUNXON = SUMD + 1E00 / (VFAC + 1.0)
...
5     CONTINUE
      ...
      DO I = 1, t__67, 1
        ...
        H1 = PTPEN(JW, 3)
        DT = (DT *(HS - H1)) /(H1 - H2)
        DT = SIGN(MIN(ABS(DT), (DTB **(1E00 / (FLOAT(I))))), DT)
...
C
      ...
      t__69 = NCP
      DO I = 1, t__69, 1
        DLI = MAX((CSXX(I, 16) + TS *(CSXX(I, 17) + CSXX(I, 18) * TS)),
     >  1.0)
