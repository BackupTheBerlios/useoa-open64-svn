...
C
      subroutine g_aifl(g_p_, kf, pipe, g_pipe, ldg_pipe, pressure, g_pr
     *essure, ldg_pressure, vapor, liquid, g_liquid, ldg_liquid)
C     ********************
C
...
C#include "common.h"
        ...
        integer g_i_, g_p_, ldg_pipe, ldg_pressure, ldg_liquid, g_jout(g
     *_pmax_), g_kntrl(g_pmax_), g_kflag(g_pmax_), g_ncps(g_pmax_), g_np
     *tp(g_pmax_)
        integer g_ncst(g_pmax_), g_nrec(g_pmax_), g_nen(g_pmax_), g_ktln
     *(g_pmax_, 15)
        real r3_v, r3_b, r2_b, g_wen(g_pmax_, 5, 12), g_pipe(ldg_pipe, *
     *), g_wten(g_pmax_, 5), g_hin(g_pmax_), g_ptpen(g_pmax_, 5, 6), g_p
     *ressure(ldg_pressure, *), g_fv(g_pmax_, 12)
        real g_fl(g_pmax_, 12), g_liquid(ldg_liquid, *), g_cost(g_pmax_,
     * 5), g_en(g_pmax_, 100), g_pfl(g_pmax_), g_tfl(g_pmax_), g_vzn(g_p
     *max_), g_tin(g_pmax_), g_fw(g_pmax_, 12)
...
C
        do ii = 1, num
          do g_i_ = 1, g_p_
            g_ptpen(g_i_, ii, 1) = g_pressure(g_i_, ii)
          enddo
