      subroutine dmsasep(nx,ny,x,fvec,bottom,top,left,right,elem)
      integer nx, ny, elem
      real fvec(*)
      real x(nx*ny), bottom(nx+2), top(nx+2),
...
c     **********
      ...
      integer i, j, k
      real area, dvdx, dvdy, hx, hy, v, vb, vl, vr, vt
...
c     triangular elements.
      ...
      do 50 j = 0, ny
         do 40 i = 0, nx
            ...
            dvdy = (vt-v)/hy
	    ELEM = ELEM + 1
            FVEC(ELEM) = area*sqrt(one+dvdx**2+dvdy**2)
   40    continue
   50 continue
