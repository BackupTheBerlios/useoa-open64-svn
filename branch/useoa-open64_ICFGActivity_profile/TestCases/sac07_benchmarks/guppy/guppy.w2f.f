...
12      CONTINUE
      END DO
      NBL = NP
      CALL bl(MACH, UREN, SFIX, AMPMAX, NBL, S, UE, DSTAR, THETA, HH,
     >  CF, AMP, CDP, CFTOT, SSEP)
...
C
      ...
      DO I = 1, t__12, 1
        ...
        RHOERI = TETINF ** GEXP2
        MUEMUI = SQRT(TETINF) *(1.5049999952 /((5.0499999523E-01 /
     >  TETINF) + 1.0))
...
6       CONTINUE
      ...
      DO I = 1, t__13, 1
        ...
        IF(I .eq. 2) THEN
          DS = DELS * 5.0E-01
          SBEG = DS + SBEG
          CALL flwtrp(SBEG, S1, M1, U1, Q1, NU1, S2, M2, U2, Q2, NU2, M
     > , U, DUDS, Q, NU)
          ...
          ELSE
            ...
            HSB = 1.5725799799
            H12(1) = (M * M * 2.8999999166E-01 +(M * M *
     >  1.1299999803E-01 + 1.0) * 2.5969998837)
          ENDIF
        ENDIF
        SINT = SBEG
1       CONTINUE
        ...
        CALL flwtrp(SINT, S1, M1, U1, Q1, NU1, S2, M2, U2, Q2, NU2, M,
     >  U, DUDS, Q0, NU)
        CALL lambl(M, U, DUDS, NU, TH, HSB, HS, HB, H, RT, CF0, DTHDX0,
     >  DHSDX0, DADX0)
        DHSB0 = DHSDX0 * DS
        DELHSB = HSBSTP
...
2       CONTINUE
        ...
        CALL flwtrp(SSM, S1, M1, U1, Q1, NU1, S2, M2, U2, Q2, NU2, M, U
     > , DUDS, Q, NU)
        CALL lambl(M, U, DUDS, NU, THM, HSBM, HS, HB, H, RT, CF, DTHDXM
     > , DHSDXM, DADXM)
        DHSB1 = DHSDXM * DS
        DTH1 = DS * DTHDXM
        ERRHSB = DHSB1 - DHSB0
        ...
        HSB1 = DHSB1 + HSB
        TH1 = DTH1 + TH
        CALL flwtrp(SS1, S1, M1, U1, Q1, NU1, S2, M2, U2, Q2, NU2, M, U
     > , DUDS, Q, NU)
        CALL lambl(M, U, DUDS, NU, TH1, HSB1, HS, HB, H, RT, CF, DTHDX1
     > , DHSDX1, DADX1)
        AMP1 = AMP + DS * 5.0E-01 *(DADX0 + DADX1)
        EPTRN1 = (HSB1 * 1.8399999619E+01 +(-2.1739999771E+01)) - ALOG(
     > RT)
        FRAC1 = 1.0
        FRAC2 = 1.0
        FRAC3 = 1.0
...
3       CONTINUE
     ...
     > , U, DUDS, Q, NU)
          ...
          HBLIM = MAX(HBSEP, HB * 1.1000000238)
          ITURB = 1
          STRAN = SBEG
          IWAK = 0
          AMP = 0.0
...
4       CONTINUE
     ...
     > , U, DUDS, Q, NU)
          HB = HBSEP
          HBLIM = HBSEP
          H = (HB + 1.0) *(M * M * TRF * 2.0000000298E-01 + 1.0) +(-1.0
     > )
          UCHG = U / U2
          IF(UCHG .LT. 1.0) THEN
            UCHG = 1.0
        ...
        ELSE
          ...
          CALL flwtrp(SINT, S1, M1, U1, Q1, NU1, S2, M2, U2, Q2, NU2, M
     > , U, DUDS, Q0, NU)
          CALL trbbl(IWAK, M, U, DUDS, NU, TH, HB, CE, H, RT, CF0,
     >  DTHDX0, DHBDX0, DCEDX0)
          DHB0 = DHBDX0 * DS
...
5         CONTINUE
          ...
          HBM = HB + DHB0 * 5.0E-01
          CEM = CE + DCE0 * 5.0E-01
          CALL flwtrp(SSM, S1, M1, U1, Q1, NU1, S2, M2, U2, Q2, NU2, M,
     >  U, DUDS, Q, NU)
          CALL trbbl(IWAK, M, U, DUDS, NU, THM, HBM, CEM, H, RT, CF,
     >  DTHDXM, DHBDXM, DCEDXM)
          DTH1 = DS * DTHDXM
          DHB1 = DHBDXM * DS
          DCE1 = DCEDXM * DS
          ERRHB = DHB1 - DHB0
          ...
          HB1 = DHB1 + HB
          CE1 = CE + DCE1
          CALL flwtrp(SS1, S1, M1, U1, Q1, NU1, S2, M2, U2, Q2, NU2, M,
     >  U, DUDS, Q, NU)
          CALL trbbl(IWAK, M, U, DUDS, NU, TH1, HB1, CE1, H, RT, CF,
     >  DTHDX1, DHBDX1, DCEDX1)
          FRAC = 1.0
...
C
      ...
      GRAD = (DUEDX * THETA) / UE
      RT = (UE * THETA) / NUE
      HS = (HSK + MSQ * 2.8000000864E-02) /(MSQ * 1.4000000432E-02 +
     >  1.0)
     ...
     >  2.271822052E+02 + 5.8360180664E+02 - HSK * 7.2455914307E+02)
          SF = (HK ** 2) * 3.9154100418E-01 + 2.5125889778 - HK *
     >  1.6860949993 -(HK ** 3) * 3.1729001552E-02
        ELSE
          HK = (HSK ** 2) * 2.571578598E+01 + 7.987084198E+01 - HSK *
     >  8.9582138062E+01
          SF = (HSK ** 2) * 2.2216870785 + 1.3723909855 - HSK *
     >  4.2262501717
        ENDIF
      ENDIF
      H = HK *(MSQ * 1.1299999803E-01 + 1.0) + MSQ * 2.8999999166E-01
      CF = (SF * 2.0) / RT
      HSS = M *((6.400000304E-02 /(HK +(-8.0000001192E-01))) +
     >  2.5099998713E-01)
      DS = (HSK ** 2) * 3.4188981056 + 7.8539757729 - HSK *
     >  1.0260549545E+01
      CD = DS / RT
      HMI = 1E00 / (HK +(-1.0))
      RTLOG = 0.0
      IF(RT .GT. 0.0) THEN
        RTLOG = ALOG10(RT)
      ENDIF
      GRCRIT = HMI * 3.2950000763 + TANH(HMI * 2.0E+01 +(
     > -1.2899999619E+01)) *(HMI * 1.4149999619 +(-4.8899999261E-01)) +
     >  4.3999999762E-01
      DADX = 0.0
      IF(GRCRIT .LT. RTLOG) THEN
        T1 = HK * 2.4000000954 +(-3.7000000477) + TANH((HK +(
     > -3.0999999046)) * 1.5) * 2.5
...
C
      ...
      CF0 = ((1.0130000301E-02 /(ALOG10(RT * FR) +(-1.0199999809))) +(
     > -7.5000000652E-04)) / FC
     ...
     > ) +(-5.0E-01))
        DCFDHB = -((CF0 * 8.9999997616E-01) /(HB0 *(((HB / HB0) +(
     > -4.0000000596E-01)) ** 2)))
        H = (HB + 1.0) *(MSQ * TRF * 2.0000000298E-01 + 1.0) +(-1.0)
        DHDHB = (MSQ * TRF * 2.0000000298E-01 + 1.0)
        H1 = (1.7200000286 /(HB +(-1.0))) + 3.1500000954 -((HB +(-1.0))
     >  ** 2) * 9.9999997765E-03
        DH1DHB = -((((HB +(-1.0)) ** 3) * 1.9999999553E-02 +
     >  1.7200000286) /((HB +(-1.0)) ** 2))
        GRDEQ0 = (1.25 / H) *(CF * 5.0E-01 -((((HB +(-1.0)) /(HB *
     >  6.4320001602)) ** 2) / FD))
        DGRDHB = (2.5 /(H * FD)) *(((((HB +(-1.0)) /(HB * 6.4320001602)
     > ) ** 2) / HB) -((HB +(-1.0)) /((HB * 6.4320001602) ** 2))) +
     >  DCFDHB *(6.25E-01 / H) - DHDHB *(1.25 /(H ** 2)) *(CF * 5.0E-01
     >  -((((HB +(-1.0)) /(HB * 6.4320001602)) ** 2) / FD))
...
C
     ...
     >  +(-5.0E-01))
      ...
      H1 = (1.7200000286 /(HB +(-1.0))) + 3.1500000954 -((HB +(-1.0))
     >  ** 2) * 9.9999997765E-03
      DHBDH1 = -(((HB +(-1.0)) ** 2) /(((HB +(-1.0)) ** 3) *
     >  1.9999999553E-02 + 1.7200000286))
     ...
     >  6.4320001602)) ** 2) / FD))
      ...
      IF(CEEQ0 .LT. CEMIN) THEN
        CEEQ0 = CEMIN
      ENDIF
      CTEQ0 = FA *(CEEQ0 * 2.4000000209E-02 +(CEEQ0 ** 2) *
     >  1.2000000477 + CF0 * 3.1999999285E-01)
      C = (CTEQ0 /(FA *(COEF ** 2))) - CF0 * 3.1999999285E-01
      CEEQ = SQRT((C / 1.2000000477) + 9.9999997474E-05) +(
     > -9.9999997765E-03)
     ...
     >  THETA
      DCEDX = (F / THETA) *(GRDEQ +(2.7999999523 /(H + H1)) *(SQRT(
     > CTEQ0) - COEF * SQRT(CT)) - GRAD *(((FB * MSQ * 7.500000298E-02)
     >  / FA) + 1.0))
      RETURN
      RETURN
      END SUBROUTINE
