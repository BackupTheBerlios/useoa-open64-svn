...
!!
!     Stub to be used to designate a def side-effect proc on a parameter
      subroutine defProcParam(junk)
      integer, intent(out) :: junk
      integer junk2
...
c      call timer_clear(T_bench)
      !defProcParam(T_bench), mms timer_clear must be defining some global var
c      call timer_clear(T_init)
      !defProcParam(T_init)

      call mpi_barrier(MPI_COMM_WORLD, ierr)

c      call timer_start(T_init)
      !defProcParam(T_init)
...
c---------------------------------------------------------------------
 ...
 1000 format(//,' NAS Parallel Benchmarks 3.1 -- MG Benchmark', /)
...
c      call timer_stop(T_init)
      !defProcParam(T_init)
c      call timer_start(T_bench)
      !defProcParam(T_bench)
...
c      call timer_stop(T_bench)

c      t0 = timer_read(T_bench)
c      tinit = timer_read(T_init)
...
C          call print_results('MG', class, nx(lt), ny(lt), nz(lt), 
C      >                      nit, nprocs_compiled, nprocs, t,
...
!      integer n1,n2,n3,k
!      BKXFORM - commented line above, redefined below
      integer, intent(out) :: n1, n2, n3, k
...
!      BKXFORM - swap 2 below for 1 above ?
!      call useProcParam(nprocs)
!      call defProcParam(log_p)
      ...
      idi(1) = mod(me,pi(1))
!      BKXFORM - swap 2 below for 1 above ?
!      call use2ProcParam(me,pi(1))
!      call defProcParam(idi(1))
...
!      BKXFORM - swap 2 below for 1 above ?
!      call use3ProcParam(me,pi(1),pi(2))
!      call defProcParam(idi(2))
...
!            BKXFORM of above
!            call use4ProcParam(ax,idi(ax),next(ax),pi(ax))
!            call defProcParam(idin(ax,+1))
...
!            call defProcParam(idin(ax,-1))
     ...
     >           * idin(3,dir))
      ...
      if( debug_vec(1) .ge. 1 )then
         if( me .eq. root ) then
            write(*,*)' in setup, '
!         BKXFORM - commented above, and added endif
         endif

         if( me .eq. root )then

         write(*,*)' me   k  lt  nx  ny  nz ',
...
!         BKXFORM - commented above, and added endif
     ...
     >              n1,n2,n3,is1,is2,is3,ie1,ie2,ie3
 9             format(15i4)
!         BKXFORM - commented above,and added uses below:
!               call use5ProcParam(me,k,lt,ng(1,k),ng(2,k))
!               call use5ProcParam(ng(3,k),n1,n2,n3,is1)
!               call use5ProcParam(is2,is3,ie1,ie2,ie3)
      ...
      if( debug_vec(1) .ge. 2 )then
         do  i=0,nprocs-1
            if( me .eq. i )then
               ...
               write(*,*)' '
               write(*,*)' processor =',me
!          BKXFORM - commented 2 above, added use below
!               call useProcParam(me)
     ...
     >                 (mi(d,k),d=1,3)
!          BKXFORM - commented3 above, added uses with loops below (6lines)
!                  call use4ProcParam(k,idi(1),idi(2),idi(3))
!                  do j=-1,1,2
!                     do d=1,3
!                        call use4ProcParam(d,j,k,nbr(d,j,k))
!                     enddo
!                  enddo
...
!          BKXFORM - commented above, previously removed use of 7

               write(*,*)'idi(s) = ',(idi(s),s=1,3)
!          BKXFORM - commented above, added loop below
!               do s=1,3
!                  call use2ProcParam(s,idi(s))
!               enddo

               write(*,*)'dead(2), dead(1) = ',dead(2),dead(1)
!          BKXFORM - commented above, added use below
!               call use2ProcParam(dead(2),dead(1)
...
!          BKXFORM - commented 2 above, added use below
!                  call use3ProcParam(ax,give_ex(ax,2),take_ex(ax,2))
...
c$openad INDEPENDENT(c)
c$openad DEPENDENT(u)
...
c---------------------------------------------------------------------
      ...
      e3 = ie3 - is3 + 2
      x0 = x
c      rdummy = randlc( x0, ai )
      do  i3 = 2, e3
         x1 = x0
         do  i2 = 2, e2
            xx = x1
            !call vranlc( d1, xx, a, z( 2, i2, i3 ))
            !MMS, don't know what this function does so make it mod
            !all params
            call defProcParam(d1)
            call defProcParam(xx)
            call defProcParam(a)
            call defProcParam(z(2,i2,i3))
c            rdummy = randlc( x1, a1 )
         enddo
c         rdummy = randlc( x0, a2 )
      enddo
...
c---------------------------------------------------------------------
 ...
 100  continue
      ...
      if( nj .eq. 0 ) goto 200
c      if( mod(nj,2) .eq. 1 ) rdummy =  randlc( power, aj )
c      rdummy = randlc( aj, aj )
      nj = nj/2
      go to 100
