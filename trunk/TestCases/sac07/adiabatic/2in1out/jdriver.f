...
c
      ...
      real g_pipe(pmax,6), g_pressure(pmax,6), g_liquid(pmax,6)
...
c    ADIFOR Initializations
      ...
      ldg_liquid = pmax
c
c    Initialize seed matrices for ADIFOR
c
       call initeye (g_p,ncps,g_pipe,pmax)
       do i = 1, 5
          do j = 1, g_p
             g_pressure(j,i) = 0.
          enddo
       enddo
       g_pressure(g_p,3) = 1.0
c

c
c     Call ADIFOR generated subroutine
c
       call g_aifl(g_p, 1, pipe, g_pipe, ldg_pipe, pressure, 
     *      g_pressure, ldg_pressure, vapor, liquid, g_liquid, 
     *      ldg_liquid)
      ...
      write(*,*) 
     +'AD liquid (first 6 rows d/d(pipe), last row d/d(pressure)'
      write(*,1111) ((g_liquid(i,j),j=1,6),i=1,7) 
      write(*,*)'*******************************************'
