...
c
      ...
      subroutine fdjs(m,n,col,ind,npntr,ngrp,numgrp,eta,fjacd,fjac)
      integer m,n,numgrp
      integer ind(*),npntr(*),ngrp(n)
      real eta(n),fjacd(m),fjac(*)
      logical col
c     **********
c
c     subroutine fdjs
...
c     parameter vector eta and an approximation to J*d where J
c     is the Jacobian matrix and the vector d is defined by the
...
c        10    continue
c
c     In the above code numgrp is the given group number, ngrp(j)
c     is the group number of column j, and eta(j) is the difference
c     parameter used to approximate column j of the Jacobian matrix.
c     Suitable values for eta(j) must be provided.
c
c     As mentioned above, an approximation to J*d must be provided.
c     For example, if f(x) is the function value at x, then
c
c           f(x+d) - f(x)
c
...
c     The subroutine statement is
c
c       subroutine fdjs(m,n,col,ind,npntr,ngrp,numgrp,eta,fjacd,fjac)
c
c     where
c
c       m is a positive integer input variable set to the number
...
c
c       fjacd is an input array of length m which contains an
c         approximation to J*d, where J is the Jacobian matrix
c         and d is the difference vector for group numgrp.
c
c       fjac is an output array of length nnz, where nnz is the
c         number of non-zero elements in the Jacobian matrix.
c         At each call of fdjs, fjac is updated to include the
c         non-zero elements of the Jacobian matrix for those columns
c         in group numgrp.
c
c     Argonne National Laboratory. MINPACK Project. December 1984.
c     Thomas F. Coleman, Burton S. Garbow, Jorge J. More'
...
c     to J*d, where J is the Jacobian matrix and d is the
c     difference vector for group numgrp.
c
      if (col) then
c
...
c
         do 20 j = 1, n
            if (ngrp(j) .eq. numgrp) then
               do 10 jp = npntr(j), npntr(j+1)-1
                  i = ind(jp)
                  fjac(jp) = fjacd(i)/eta(j)
   10             continue
               end if
   20       continue
      else
...
c
         do 50 i = 1, m
            do 30 ip = npntr(i), npntr(i+1)-1
               j = ind(ip)
               if (ngrp(j) .eq. numgrp) then
                  fjac(ip) = fjacd(i)/eta(j)
                  go to 40
                  end if
   30          continue
   40       continue
